...
if (!empty($_POST['register'])) {
    ...
    if ($fetchDataUserEmail['email'] == $daftar_email) {
        $daftar->add_with_type('Email Sudah pernah Digunakan,Silahkan Login', 'danger', '../auth/register.php');
    } elseif ($fetchDataUserEmail['username'] == $daftar_username) {
        $daftar->add_with_type('Username Sudah pernah Digunakan Silahkan Login', 'danger', '../auth/register.php');
    } else {
        ...
        if ($insertToToken) {
            $daftar->add_with_type('Pendaftaran Berhasil, Silahkan Login', 'success', '../auth/login.php');
            ...
            while ($insertToUser) {
                $daftar->kirim_email($daftar_email, $token);
            }
        } else {
